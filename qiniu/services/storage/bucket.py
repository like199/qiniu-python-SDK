...
class BucketManager(object):
    ...
    def restoreArchive(self, bucket, key, freeze_after_days):
        """解冻归档存储文件
        ...
        """
        resource = entry(bucket, key)
        return self.__rs_do('restoreAr', resource)
    ...
    def rm_bucket(self, bucket_name):
        ...
        Args:
            ...
            region: 存储区域
        """
        return self.__rs_do('drop', bucket_name)
    ...
    def set_bucketTagging(self, bucket_name, body):
        """
        设置 存储空间标签
        ...
        """
        url = "{0}/bucketTagging?bucket={1}".format(config.get_default("default_uc_host"), bucket_name)
        return self.__put(url, body)

    def list_bucketTagging(self, bucket_name):
        """
        查询 存储空间标签
    ...
    def delete_bucketTagging(self, bucket_name):
        """
        查询 存储空间标签
        https://developer.qiniu.com/kodo/api/6315/get-bucket-tagging
        Args:
    ...
    def set_bucketImagesource(self, bucket_name, resourceDomain, resourceHost=None):
        """
        设置 存储空间 镜像源
        ...
        if resourceHost is None or resourceHost == "":
            url = "{0}/image/{1}/from/{2}".format(config.get_default("default_uc_host"), bucket_name, resourceDomain)
        else:
            url = "{0}/image/{1}/from/{2}/host/{3}".format(config.get_default("default_uc_host"), bucket_name,
    ...
    def __put(self, url, data=None):
        return http._put_with_qiniu_mac(url, data, self.auth)
